...
class VAE(tf.keras.Model):
    def __init__(self):
        ...
        self.input_size = (128, 128)  # height * width * num_images
        self.latent_size = 10000  # Z
        self.hidden_dim = 20000  # H_d
        ...
        self.encoder = Sequential([
            ...
            tf.keras.layers.Conv2D(filters=32, kernel_size=5, strides=(1, 1), activation='relu'),  # out: (124,124)
            tf.keras.layers.Conv2D(filters=32, kernel_size=5, strides=(1, 1), activation='relu'),  # out: (120,120)
            tf.keras.layers.MaxPool2D(pool_size=(2, 2)),  # out: (60,60)
            tf.keras.layers.Conv3D(filters=64, kernel_size=3, strides=(1, 1), activation='relu'),  # out: (56,56)
            tf.keras.layers.Conv3D(filters=64, kernel_size=3, strides=(1, 1), activation='relu'),  # out: (52,52)
            tf.keras.layers.MaxPool2D(pool_size=(2, 2)),  # out: (26,26)
            tf.keras.layers.Conv3D(filters=128, kernel_size=3, strides=(1, 1), activation='relu'),  # out: (22,22)
            tf.keras.layers.Conv3D(filters=128, kernel_size=3, strides=(1, 1), activation='relu'),  # out: (18,18)
        ...
        self.decoder = Sequential([
            ...
            tf.keras.layers.Dense(units=28*28*32),
            tf.keras.layers.Reshape(target_shape=(28, 28, 32)),
            tf.keras.layers.Conv2DTranspose(filters=128, kernel_size=4, strides=(2, 2), activation='relu'),
            tf.keras.layers.Conv2DTranspose(filters=64, kernel_size=3, strides=(1, 1), activation='relu'),
            tf.keras.layers.Conv2DTranspose(filters=32, kernel_size=6, strides=(2, 2), activation='relu'),
            tf.keras.layers.Conv2DTranspose(filters=1, kernel_size=5, strides=(1, 1)),
        ])
